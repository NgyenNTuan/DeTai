...
public class GiaoDienThueBangDia extends JFrame implements ActionListener {
	...
	public GiaoDienThueBangDia(String maKH, String maNV) {
		...
		btnThemGioHang
				.setIcon(new ImageIcon("C:\\DeTaiPhatTrienUngDung\\QuanLyThueBangDia\\icon\\icons8-arrow-right-55.png"));
	...
	private void capNhatBangDuLieu() {
		...
		for (BangDia bangDia : list) {
			...
			String[] rowData = { bangDia.getMaBangDia(), bangDia.getTenBangDia(), bangDia.getTheLoai(), soLuong + "",
		...
		if (o.equals(btnThemGioHang)) {
			...
			if (k != 0) {
				if (HD.themHoaDon(maHD, maNV, maKH)) {
					...
					GiaoDienChiTietPhieuThue pt = new GiaoDienChiTietPhieuThue(maHD, 0);
					pt.setVisible(true);
					setVisible(false);
				}
			}
